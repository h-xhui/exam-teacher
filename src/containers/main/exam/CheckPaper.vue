      ...
      <el-card>
        ...
        <div class="box" v-for="(item,index) in testForm" :key="index" >
          ...
          <div style="margin-left:80%">
            <span>学生得分: </span>
            <el-input v-if="item.type !== 1 && item.type !== 2 && item.type !== 3" class="score" v-model="scoreForm[index]" placeholder="请输入得分" type="Number"></el-input>
            <span v-else style="color:blue"> ({{item.queScore}}分)</span>
          </div>
...
export default {
  data(){
    return{
      scoreForm:this.$route.params.scores,
